/*  
    Proxy API Server for JupyterHub and LTIConrainerSpawner
...
#define  LTICTR_ALLOW_FILE  "TICTR_Allow_File"
#define  LTICTR_PID_FILE    "TICTR_Pid_File"
#define  LTICTR_SERVER_CERT "TICTR_Server_Cert"
#define  LTICTR_PRIVATE_KEY "TICTR_Private_Key"
...
int main(int argc, char** argv)
{
    ...
    if (aport==0) {
        ...
        print_message("            [--allow allow_file] [--apid pid_file] [--conf config_file]  [--cert cert_file] [--key key_file]\n");
        sig_term(-1);
    }
    //
...
int  init_main(Buffer configfile)
{
    ...
    if (AllowList!=NULL) {
        DEBUG_MODE print_message("[LTICTR_API_SERVER] Readed access allow list\n");
    }
    else {
        DEBUG_MODE print_message("[LTICTR_API_SERVER] Unable to read access allow list. No access control is performed.\n");
    }
